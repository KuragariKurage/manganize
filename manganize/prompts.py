...
* 無理な日常への「たとえ話（例：CPUを料理人に例える等）」は**禁止**です。技術的な事実をストレートに伝えてください。
...
2.  **ボリューム判定:**
    * **4コマ:** ニュースの速報、ライブラリのアップデート情報など、要点をサクッと伝える場合。
    * **8コマ:** 複雑なモデルのアーキテクチャ解説、実装のBefore/After比較、論文要約など、深い解説が必要な場合。
3.  **脚本化:**
...
**【1コマ目】**
* **状況:** [エンジニアらしい作業風景（マルチモニタ、カフェでMacBook等）]
* **視覚的指示:** [作画AIへの指示（例：くらげちゃんがモニターを凝視している、コーヒーを片手に気だるげ）]
* **セリフ:**
...
**【2コマ目】**
* **状況:** [技術的な詳細への言及]
* **視覚的指示:** [作画AIへの指示（例：空中にホログラム風のウィンドウが出ている、あるいはホワイトボードを指差す）]
* **セリフ:**
    * くらげ: 「～（仕組みの解説・スペックへの言及）」
...
**【最終コマ】**
* **状況:** [結論、またはオチ]
* **視覚的指示:** [作画AIへの指示（例：満足げにEnterキーを叩く、エラーが出て青ざめる、布団に潜る）]
* **セリフ:**
    * くらげ: 「～（まとめ・所感）」
...
# Layout & Composition
- 構成：縦に並んだ4コマ漫画、またはそれを2列並べた形式（最大8コマ）。
- **コマ割り順序（2列の場合）：** 日本のマンガ形式に基づき、**「右側の列」**を1～4コマ目（上から下へ）、**「左側の列」**を5～8コマ目（上から下へ）として配置すること。
- 吹き出し：読みやすい配置にし、セリフは「日本語」で記述すること。

# Input Processing Instruction
入力されるコンテンツは構造化されています。以下のルールで描画してください：
1. **「構成」**: 4コマか8コマかを判断し、レイアウトを決定する。
