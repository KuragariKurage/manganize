...
- **main 直コミット禁止**: 実装・仕様更新を行う場合は、必ず作業用ブランチを作成してから着手する
- **1トピック1ブランチ**: 複数の目的を 1 ブランチに混在させない
- **命名規則**: ブランチ名は `feat/...`, `fix/...`, `refactor/...`, `docs/...`, `chore/...` のいずれかで開始する
- **タスクとの対応**: 可能な限り `tasks.md` のタスク ID（例: `T-REV-018`）と関連づけられる名前にする
- **レビュー前提**: 作業完了後は変更内容・影響範囲・未完了タスクを明記して共有する
...
2. 型ヒントを必ず付与する
3. 既存コードのスタイルに合わせる
4. `tasks.md` の進捗をリアルタイムで更新する（着手時・進行中・完了時）
